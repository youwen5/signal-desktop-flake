{
  ...
  # electron, so it can be removed once Signal picks it up
...
let
  ...
  url = "https://download.copr.fedorainfracloud.org/results/useidel/signal-desktop/fedora-42-aarch64/08759579-signal-desktop/signal-desktop-${version}.fc42.aarch64.rpm";
  ...
  copy-noto-emoji = fetchurl {
    ...
    hash = "sha256-bunpZ/pQ1yZskJJiyFdhFhu9jxaRHlsSax83fHTQ3gM=";
  };
...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    ...
    postFetch = ''
      ...
      rm -r \
        "$out/usr/lib64/${pname}/resources/app.asar"{,.unpacked} \
  ...
  installPhase = ''
    ...
    mv "usr/lib64/${pname}" "$out/lib/${dir}"
  ...
  preFixup = ''
    ...
    substituteInPlace $out/share/applications/${pname}.desktop \
      --replace-fail "/usr/bin/${pname}" ${meta.mainProgram} \
